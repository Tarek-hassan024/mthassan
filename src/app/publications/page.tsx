...
function renderHighlightedCitation(text: string) {
  return text.split(/(M\. T\. Hassan|Md Tarek Hassan)/g).map((part, idx) =>
    part === "M. T. Hassan" || part === "Md Tarek Hassan" ? (
...
export default function PublicationsPage() {
  ...
  const submittedAndUnderReview = publications.filter(
    (p) => p.status === "Submitted" || p.status === "Under Review"
  );
  const sections = [
    { title: "Submitted and Under Review", items: submittedAndUnderReview },
    { title: "Accepted Papers", items: accepted },
  ...
  return (
    <Container className="pt-10">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Card>
          <div className="text-xs text-white/60">Submitted and Under Review</div>
          <div className="mt-1 text-2xl font-semibold">{submittedAndUnderReview.length}</div>
        </Card>
        <Card>
                  ...
                  .map((p, idx) => (
                    <Card key={`${p.titleLine}-${idx}`}>
                      ...
                      <p className="mt-3 text-sm text-white/80 leading-relaxed">
                        {renderHighlightedCitation(p.titleLine)}
                      </p>
